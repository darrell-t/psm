...
int waterTempFlag = 1, airTempFlag = 1, lightIntFlag = 1, airHumFlag = 1; // To trigger the send email function
float wTempMax, wTempMin, aTempMax, aTempMin, aHumMax, aHumMin, lightMax, lightMin; // Sensor Threshold range
...
void sendReadings(struct sensorCondition waterTemp, struct sensorCondition airTemp, struct sensorCondition airHum, struct sensorCondition lightInt){
  HTTPClient http;
  String postData;
...
void loop() {
  struct sensorCondition waterTemp, airTemp, lightInt, airHum;
  String payload;
  ...
  sendReadings(waterTemp, airTemp, airHum, lightInt);
  delay(5000);
}
